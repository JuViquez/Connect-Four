...
class GeneticAlgorithm:
    ...
    def population_generator(self):
        for i in range(self.population_size):
            strategy = ConnectFourStrategy(
                self.randomness_provider,
                ConnectFourChecker(),
                ConnectFourScore(),
                ConnectFourSearcher())
            prob_list = self.randomness_provider.gen_rand_prob_list(8)
            agent = ConnectFourAgentChromosome(
                "R", self.randomness_provider, strategy, prob_list)
        ...
        for agent1 in range(self.population_size):
            for agent2 in range(agent1 + 1, self.population_size):
                if agent1 != agent2:
                    ...
                    match = ConnectFour(
                        self.population[agent1][0],
                        self.population[agent2][0])
    ...
    def cross_controller(self):
        probabilities = self.randomness_provider.scale_probs(
            [i[1] for i in self.population])
        index_list = list(range(self.population_size))
        children = []
        for i in range(self.children_size):
            agent1 = self.randomness_provider.prob_choice(
                index_list, probabilities)
            agent2 = self.randomness_provider.prob_choice(
                index_list, probabilities)
            while agent1 == agent2:  # Para que no se repitan
                agent2 = self.randomness_provider.prob_choice(
                    index_list, probabilities)
            child = self.population[agent1][0].cross(
                self.population[agent2][0].strategies_probs)
