...
def test_cross_controller(genetic):
    ...
    # Se cambia el random provider para hacer el cruce
...
def test_mutate_controller(genetic):
    ...
    genetic.randomness_provider = FakeRandomnessProvider(
        0)  # se restaura el random provider
    genetic.mutate_controller(children)
    assert all(
        i != j for (
            i,
            j) in zip(
            children[0][0].strategies_probs,
            child_chromosome))
